...
self.addEventListener('install', e => {
    const appShell = caches.open(CACHE_STATIC_NAME)
        .then(cache => {
            // Agregar App Shell
            return cache.addAll([
                '/pwa-training/5-estrategias-de-cache-y-offline-mode/11-cache-y-network-race/',
                'index.html',
                'css/style.css',
                'img/main.jpg',
                'js/app.js',
...
self.addEventListener('fetch', e => {
    ...
    // 5- Cache y Network Race: el primero de los dos que se resuelva, va a
    // ser la respuesta a esa solicitud
